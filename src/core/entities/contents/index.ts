...
const byRef = (
  ...
  switch (action.type) {
    ...
    case actionTypes.FETCH_CONTENT_LOADED:
      const fetchContentLoadedAction = action as actionTypes.FetchContentLoaded;
      switch (fetchContentLoadedAction.payload.model.type) {
        case "notebook": {
          const immutableNotebook = fromJS(
            fetchContentLoadedAction.payload.model.content
          );

          return state.set(
            fetchContentLoadedAction.payload.contentRef,
            makeNotebookContentRecord({
